...
def init_openai_client():
    try:
        # Попробуем новый способ инициализации
        client = OpenAI(api_key=API_KEY, base_url=BASE_URL)
        print("✅ OpenAI клиент инициализирован с base_url")
        return client
    except Exception as e:
        print(f"⚠️ Ошибка с base_url: {e}")
        try:
            # Fallback - только с API ключом
            client = OpenAI(api_key=API_KEY)
            print("✅ OpenAI клиент инициализирован без base_url")
            return client
        except Exception as e2:
            print(f"❌ Критическая ошибка инициализации OpenAI: {e2}")
            raise e2
...
class LLMProcessor:
    ...
    def handle_employee_query(self, user_query: str, employee_id: int, max_steps: int = 5):
        ...
        try:
            ...
            return f"Извините, произошла ошибка при генерации финального ответа: {str(e)}"
    ...
    def handle_manager_query(self, manager_query: str):
        ...
        try:
            ...
            print(f"   - Не удалось извлечь структурированные критерии: {e}")
            criteria = {}
        ...
        **Твоя задача:**
        ...
        try:
            ...
            return f"Извините, произошла ошибка при генерации отчета: {str(e)}"
